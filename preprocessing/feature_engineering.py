...
class TimeSeriesFeatureEngineering:
    ...
    '''
    def __init__(self, x_columns, roll_shift = 10, ts_variable = 'timestamp', y_column = "Y"):
    ...
    def make_features(self, frame):
        ...
        features_frame[self.y_column] = labels.iloc[features_frame.index][self.y_column]
        return features_frame

    def extract(self, frame, labels, x_columns, y_column):
        extract_columns = ['tsfresh_id', 'sort'] + x_columns
        features = tsfresh.extract_features(frame[extract_columns], column_id='tsfresh_id', column_sort='sort', disable_progressbar=True)
        features = features.mask(np.isinf(features))
        features.fillna(value=0, inplace=True)
        selected_features = select_features(features, labels[y_column], ml_task = "classification")
    ...
    def aggregate_labels(self, rolled_series, y_column):
        ...
        the_max_class = class_and_roll.groupby(by=["tsfresh_id"], as_index=False).max()
